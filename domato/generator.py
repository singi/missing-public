...
def generate_samples(outfile):
    ...
    template = random.choice(glob.glob( parent_dir + "/templates/*.html"))    
    grammar_dir = parent_dir + "/grammar/"
    grammar_name = os.path.basename(template).replace(".html", ".txt")
    #템플릿 이름이 webgl.html 이라면, 항상 webgl.txt 문법을 읽어옴.
    #여러 템플릿을 쓰려면, webgl1.html, webgl2.html, webgl3.html을 만들고 webgl.txt를 webgl1.txt, webgl2.txt, webgl3.txt 로 생성.


    f = open(os.path.join(grammar_dir, template))
    template = f.read()
    f.close()    

    jsgrammar = Grammar()
    err = jsgrammar.parse_from_file(os.path.join(grammar_dir, grammar_name))
    if err > 0:
